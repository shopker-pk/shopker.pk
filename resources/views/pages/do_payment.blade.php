                        ...
                        <div class="col-xs-12 col-sm-6 col-md-2 col-lg-2">
                            <div class="panel panel-default">
								...
								<form method="post" action="https://payments.jazzcash.com.pk/CustomerPortal/transactionmanagement/merchantform">  
									{{ csrf_field() }}
									<div id="jazz_data" data-merchant-id="{{ $merchant_id = '00157845' }}" data-password="{{ $password = '30w85gd196' }}" data-return-url="{{ $return_url = 'https://shopker.pk/update-jazz-order' }}" data-hask-key="{{ $hash_key = 'zy3w009u85' }}" data-amount="{{ $amount = Session::get('shipping_details')['total'].'00' }}" data-bill-reference="{{ $bill_reference = '00001' }}" data-description="{{ $description = '00001' }}" data-language="{{ $language = 'EN' }}" data-currency="{{ $currency = 'PKR' }}" data-date-time="{{ $date_time = date('YmdHis') }}" data-expiry-date-time="{{ $expiry_date_time = date('YmdHis', strtotime('+8 Days')) }}" data-ref-no="{{ $ref_no = 'TXN'.date('YmdHis') }}" data-type="{{ $type= '' }}" data-version="{{ $version = '1.1' }}" $data-sub-merchant-id="{{ $data_sub_merchant_id = '' }}" data-discounted-amount="{{ $discounted_amount = '' }}" data-discounted-bank="{{ $discounted_bank = '' }}" data-ppmpf-1="{{ $ppmpf_1 = '' }}" data-ppmpf-2="{{ $ppmpf_2 = '' }}" data-ppmpf-3="{{ $ppmpf_3 = '' }}" data-ppmpf-4="{{ $ppmpf_4 = '' }}" data-ppmpf-5="{{ $ppmpf_5 = '' }}" data-hash-array="{{ print_r($HashArray = [$amount,$bill_reference,$description,$discounted_amount,$discounted_bank,$language,$merchant_id,$password,$return_url,$currency,$date_time,$expiry_date_time,$ref_no,$type,$version,$ppmpf_1,$ppmpf_2,$ppmpf_3,$ppmpf_4,$ppmpf_5]) }}" data-sorted-array="{{ $sorted_array = $hash_key }}" data-loop="<?php for($i = 0; $i < count($HashArray); $i++){ if($HashArray[$i] != 'undefined' && $HashArray[$i] != null && $HashArray[$i] != ""){$sorted_array .='&'.$HashArray[$i];}}?>" data-secure-hash="{{ $Securehash = hash_hmac('sha256', $sorted_array, $hash_key) }}"></div>
